...
3. Choose Chinese or English and decide whether to include an AI-generated scope.
4. Stage files and select the compact **AI** button beside the commit field. The model always infers a type such as `feat`, `fix`, or `docs` from the staged diff. When scope is enabled, the model also chooses the scope name. RepoPuck fills the draft but never commits or pushes automatically.

RepoPuck validates the selected format: scope off produces `feat: subject`, while scope on produces `feat(ui): subject`, with a 72-character limit. The type and, when enabled, scope name are inferred from staged content. You can edit every generated draft before committing.
