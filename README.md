...
- ⚡ **低开销刷新**：仓库未变化时只检查轻量状态指纹，避免重复构建完整快照。
...
- 🔄 Fetch、Pull、Push、Stash 与刷新状态；长时间 Fetch 会显示耗时并可安全取消。
- 🗂️ 在资源管理器或终端中打开仓库。
...
3. 选择生成中文或英文提交信息，并决定是否使用 AI 生成的作用域。
4. 暂存文件后，点击提交框旁紧凑的 **AI** 按钮。AI 始终根据已暂存差异判断 `feat`、`fix`、`docs` 等类型；开启作用域时，作用域名称也由 AI 判断。生成结果只会填入草稿，不会自动 Commit 或 Push。

RepoPuck 会按你的选择验证并统一格式：关闭作用域时生成 `feat: 说明`，开启时生成 `feat(ui): 说明`（最长 72 个字符）。其中提交类型以及开启后的作用域名称均由 AI 根据已暂存内容判断；你仍可在提交前自由编辑结果。

> [!IMPORTANT]
> API Key 按服务 Origin 分别保存在当前 Windows 用户的**凭据管理器**中，不会写入 `settings.json`、前端存储、日志或仓库。切换服务主机后必须明确确认已有密钥或重新保存；旧密钥不会被静默发送。只有主动点击生成时，RepoPuck 才会把有大小上限的**已暂存文本差异**直接发送到你配置的服务；设置页会预览文件数量、近似字节数、二进制省略、敏感路径排除和截断状态。自动过滤无法识别所有秘密，请在使用第三方服务前阅读其隐私政策，并检查即将发送的暂存内容。
...
只保存非敏感的便捷设置：主题、固定状态、桌面模式、面板尺寸、入口位置、显示器选择、顶部卷轴位置、有限数量的最近仓库路径，以及 AI 服务地址、模型、语言和是否使用作用域的格式偏好。不保存提交内容、仓库文件内容或鼠标历史；可选的 AI API Key 单独保存在 Windows 凭据管理器。
...
- [x] ⚡ 状态指纹刷新、非阻塞远端操作与可取消 Fetch
- [x] 🔐 按 Provider 隔离的 AI 凭据与发送摘要
- [x] 📦 Windows MSI 静默安装/卸载冒烟测试
