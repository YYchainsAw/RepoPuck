...
3. 选择生成中文或英文提交信息。
4. 暂存文件后，点击提交框旁紧凑的 **AI** 按钮。AI 会根据已暂存差异自动判断 `feat`、`fix`、`docs` 等类型及可选 scope；生成结果只会填入草稿，不会自动 Commit 或 Push。

RepoPuck 会验证 AI 返回的完整 Conventional Commit，并统一为 `feat: 说明` 或 `feat(ui): 说明` 这样的单行格式（最长 72 个字符）。你仍可在提交前自由编辑结果。
...
只保存非敏感的便捷设置：主题、固定状态、桌面模式、面板尺寸、入口位置、显示器选择、顶部卷轴位置、有限数量的最近仓库路径，以及 AI 服务地址、模型和语言。不保存提交内容、仓库文件内容或鼠标历史；可选的 AI API Key 单独保存在 Windows 凭据管理器。
